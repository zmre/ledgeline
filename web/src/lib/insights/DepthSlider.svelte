<!-- Account-depth slider (WP-05): daisyUI range with tick labels, 1..max. -->
<script lang="ts">
    let {depth = $bindable(), max}: {depth: number; max: number} = $props();

    const ticks = $derived(Array.from({length: Math.max(1, max)}, (_, i) => i + 1));
</script>
