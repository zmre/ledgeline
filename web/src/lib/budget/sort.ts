...
/**
 * Read a rule's recurrence.
 *
 * ONE accessor, deliberately: plan 21 reshapes `BudgetRule.period` from a bare
 * string into `{raw, simple}`, and this function is the single line that has to
 * change when it does. Nothing else in the sorting path touches the field.
 */
