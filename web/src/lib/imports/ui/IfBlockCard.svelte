...
<div class="card border border-base-content/10 bg-base-200" data-testid="imports-rule">
    <div class="card-body gap-3 p-3">
        <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-base-content/60">Rule {position}</span>
        ...
        <div class="flex flex-col gap-2">
            ...
            {#each rule.matchers as matcher, index (index)}
                <div class="flex items-center gap-2">
                    <select class="select w-40 shrink-0 select-xs" {disabled} aria-label="Rule {position}, match {index + 1} column" bind:value={matcher.field}>
            ...
            {/each}
            <button type="button" class="btn gap-1 self-start btn-ghost btn-xs" {disabled} onclick={addMatcher}>+ Add match</button>
        </div>
        ...
        <div class="flex flex-col gap-2">
            ...
            {#each rule.assignments as assignment, index (index)}
                <div class="flex items-center gap-2">
                    <select class="select w-40 shrink-0 select-xs" {disabled} aria-label="Rule {position}, set {index + 1} field" bind:value={assignment.field}>
