...
<details class="dropdown" bind:open use:dismissible={{active: open, onDismiss: () => (open = false), outside: true}}>
    ...
    <div class="dropdown-content z-20 mt-1 w-72 max-w-[calc(100vw-2rem)] rounded-box bg-base-200 p-2 shadow-lg">
...
{#snippet nodes(list: AccountNode[], depth: number)}
    {#each list as node (node.fullName)}
        ...
        <li>
            <label class="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-base-300" style="padding-left: {0.5 + depth}rem">
                <input
                    type="checkbox"
