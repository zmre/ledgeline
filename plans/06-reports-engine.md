...
export interface PeriodReport { buckets: string[]; rows: { account: string; depth: number; values: MixedAmount[] }[]; totals: MixedAmount[] }  // cf, net worth
```
...
export function netWorth(txns: Transaction[], prices: PriceDb, opts: { end: ISODate; interval: Interval; count: number }): PeriodReport;

// prices.ts
export interface PriceDb { lookup(commodity: string, asOf: ISODate): Amount | null }  // latest P directive ≤ asOf; direct conversions only (chain conversion = documented post-MVP TODO)
export function buildPriceDb(directives: PriceDirective[]): PriceDb;
export function valueAt(ma: MixedAmount, target: string, db: PriceDb, asOf: ISODate): Dec; // convert each commodity via db (mul), identity if same; unpriced commodities carried at... FLAG: skip + report in result meta, do not guess
```
