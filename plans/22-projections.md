...
### 18. An EVENT posting's amount is SIGNED; only a recurring line's is a magnitude
...
### 19. The SPA sends no `asOf`, and `Projection.start` is what the tab shows
...
### 20. `DECODERS` became `[name, decoder, body]` triples
...
### 21. `PeriodLineChart` gained `includeZero` and `markAt`
...
### 22. A stale projection is kept on screen and LABELLED, not replaced by a spinner
...
### 23. Decisions the plan left open
...
### 24. `LedgelineApi.mutate` forwards `extraStatuses`
...
### 25. Phase 3 seams, already in place
...
### 26. Component-test fixtures are inlined wire bytes
