...
| `two-accounts.ofx` | A **download-all-my-accounts** file: two `STMTTRNRS`/`STMTRS` under one `BANKMSGSRSV1`, checking (one transaction) and savings (two). Only the first is read — each statement's rows belong to its own account and a rules file names one `account1`, so merging would post the savings rows to checking — and the rest must be REPORTED, as `StatementChosen { of: 2 }`. The arithmetic net cannot catch this alone: statement one reconciles perfectly against its own `LEDGERBAL` while two thirds of the file is missing |
