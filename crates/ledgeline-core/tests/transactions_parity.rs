...
fn opening_balances_inference_and_assertion() {
    ...
    // transaction down the file: it now opens on line 110 and ends on 115.
    assert_eq!(first["tsourcepos"][0]["sourceLine"], serde_json::json!(110));
    assert_eq!(first["tsourcepos"][1]["sourceLine"], serde_json::json!(115));
